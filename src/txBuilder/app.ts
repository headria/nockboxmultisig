...
const refreshAllBtn = document.getElementById(
  "refreshAllBtn"
...
const downloadTxBtn = document.getElementById(
  "downloadTxBtn"
) as HTMLButtonElement;
const signTxBtn = document.getElementById("signTxBtn") as HTMLButtonElement;
const signedTxSection = document.getElementById(
  "signedTxSection"
) as HTMLElement;
...
const lockNameInput = document.getElementById(
  "lockNameInput"
) as HTMLInputElement;
const primitivesContainer = document.getElementById("primitivesContainer")!;
const addPrimitiveBtn = document.getElementById(
  "addPrimitiveBtn"
) as HTMLButtonElement;
const confirmAddLockBtn = document.getElementById(
  "confirmAddLockBtn"
) as HTMLButtonElement;
const cancelAddLockBtn = document.getElementById(
  "cancelAddLockBtn"
) as HTMLButtonElement;
const importLocksBtn = document.getElementById(
  "importLocksBtn"
) as HTMLButtonElement;
const exportLocksBtn = document.getElementById(
  "exportLocksBtn"
) as HTMLButtonElement;

const addPreimageBtn = document.getElementById(
  "addPreimageBtn"
) as HTMLButtonElement;
...
const preimageFileInput = document.getElementById(
  "preimageFileInput"
) as HTMLInputElement;
const addPreimageConfirmBtn = document.getElementById(
  "addPreimageConfirmBtn"
) as HTMLButtonElement;
const cancelPreimageBtn = document.getElementById(
  "cancelPreimageBtn"
) as HTMLButtonElement;
...
async function init() {
  try {
    await wasm.default("./iris_wasm_bg.wasm");
...
function createDefaultLocksWithPkh() {
  ...
  const coinbaseDefaultExists = state.locks.some(
    (l) => l.id === "coinbase-default"
  );
...
async function refreshNotesForLock(lockId: string) {
  ...
  try {
    ...
    const spendCondition = wasm.SpendCondition.fromProtobuf(
      lock.spendConditionProtobuf
    );
    const firstName = spendCondition.firstName();
    console.log(
      `Fetching notes for lock ${lockId}, firstName: ${firstName.value}...`
    );

    const balance = await state.grpcClient.getBalanceByFirstName(
      firstName.value
    );
...
function removeLock(lockId: string) {
  ...
  state.selectedInputs = state.selectedInputs.filter(
    (input) => input.lockId !== lockId
  );
...
function renderSeeds(spend: Spend): string {
  const manualSeeds =
    spend.seeds.length === 0
      ? '<div style="font-size: 0.75rem; color: #6b7280">No seeds</div>'
      : spend.seeds
          .map(
            (seed, index) => `
        <div class="seed-item">
  ...
  let autoRefundHtml = "";
  if (remaining > 0n) {
    const refundLockName =
      state.locks.find((l) => l.id === spend.input.lockId)?.name ||
      spend.input.lockId;
    autoRefundHtml = `
      <div class="seed-item" style="opacity: 0.8; cursor: not-allowed;">
        <div style="flex: 1">
          <div class="form-label" style="margin-bottom: 0.25rem">Auto Refund (Change)</div>
          <div style="font-size: 0.75rem; color: #9ca3af">${refundLockName}</div>
        </div>
        <div style="text-align: right">
          <div style="font-weight: 600">${formatNock(nicksToNock(remaining))} NOCK</div>
          <div style="font-size: 0.7rem; color: #9ca3af">Locked back to source</div>
        </div>
      </div>
    `;
  }

  return manualSeeds + autoRefundHtml;
}

function calculateSpendTotals(spend: Spend): {
  totalOutgoing: bigint;
  remaining: bigint;
} {
  const seedsTotal = spend.seeds.reduce(
    (sum, seed) => sum + seed.amount,
    BigInt(0)
  );
...
function getRemainingBalance(spend: Spend): bigint {
  const { remaining } = calculateSpendTotals(spend);
  return remaining;
}
...
function addRefundSeed(
  ...
  const refundLock = defaultLock;
  const refundSeedCondition = wasm.SpendCondition.fromProtobuf(
    refundLock.spendConditionProtobuf
  );
...
function isSpendBalanced(spend: Spend): boolean {
  ...
  console.log(
    `Spend Input Assets: ${spend.input.note.assets}, Total Outgoing (Fee + Seeds): ${totalOutgoing}`
  );
  const isBalanced = totalOutgoing <= spend.input.note.assets;
  console.log(`Is Spend Balanced? ${isBalanced}`);
  console.log("Spend Details:", spend);
  return isBalanced;
}

function getSpendBalanceText(spend: Spend): string {
  const { totalOutgoing, remaining } = calculateSpendTotals(spend);
  if (remaining === BigInt(0)) {
    return `✓ Balanced (${formatNock(nicksToNock(totalOutgoing))} NOCK)`;
  }
  if (remaining > 0n) {
    return `Change: +${formatNock(
      nicksToNock(remaining)
    )} NOCK (auto-refund)`;
  }
  return `${formatNock(nicksToNock(remaining))} NOCK`;
}
...
function removeSpend(inputId: string) {
  ...
  if (index >= 0) {
    ...
    const selectedIndex = state.selectedInputs.findIndex(
      (i) => i.id === inputId
    );
    if (selectedIndex >= 0) {
...
function balanceSeed(inputId: string, seedIndex: number) {
  ...
  if (remaining < 0n) {
    alert(
      "Insufficient funds to balance this seed. Please reduce fee or other seeds."
    );
    return;
  }
...
function updateBuilder() {
  const allBalanced =
    state.spends.length > 0 && state.spends.every(isSpendBalanced);

  console.log("Updating builder All spends balanced:", allBalanced);
  if (!allBalanced) {
    state.builder = null;
  ...
  try {
    ...
    for (const spend of state.spends) {
      ...
      const refundLockProtobuf =
        spend.seeds.length > 0
          ? state.locks.find((l) => l.id === spend.seeds[0].lockId)
              ?.spendConditionProtobuf
          : lock.spendConditionProtobuf;

      // Deserialize WASM objects from protobuf (fresh instances every time)
      const noteClone = wasm.Note.fromProtobuf(
        spend.input.note.note.toProtobuf()
      );
    ...
    console.log("Missing unlocks:", state.missingUnlocks);
    // Try to build raw tx
    try {
...
function getMissingUnlocks(builder: wasm.TxBuilder): any[] {
  ...
  for (const spend of spends) {
    ...
    console.log("Missing unlocks for spend:", missing);
...
function renderPreimages() {
  if (state.preimages.size === 0) {
    preimagesList.innerHTML =
      '<div style="font-size: 0.75rem; color: #6b7280">No preimages</div>';
    return;
  }
...
function renderUnlocks() {
  ...
  const unlocksHtml = state.missingUnlocks
    .map((unlock: any) => {
      ...
      `;
        ...
        const preimagesFor = Array.from(
          unlock.Hax.preimages_for || []
        ) as string[];
        return `
  ...
  unlocksList.innerHTML =
    unlocksHtml ||
    '<div style="font-size: 0.75rem; color: #6b7280">Unknown unlock type</div>';
}

function renderTransaction() {
  console.log("Rendering transaction...");
  console.log("Current builder:", state);
  ...
  try {
    ...
    try {
      console.log({ fees: state.builder.calcFee() });
      state.builder.validate();
    } catch (e) {
      console.log("Transaction validation failed:", e);
      isValid = false;
    ...
    if (outputs && outputs.length > 0) {
      ...
      outputsList.innerHTML =
        '<div style="font-size: 0.75rem; color: #6b7280">No outputs</div>';
    }
...
document.addEventListener("click", (e: Event) => {
  ...
  switch (action) {
    ...
    case "select-note":
      addInputToSpend(
        target.dataset.lockId!,
        parseInt(target.dataset.noteIndex!)
      );
      break;
    case "update-fee":
...
document.addEventListener("input", (e: Event) => {
  ...
  switch (action) {
    ...
    case "update-pkh-addr":
      if (prim.pkh)
        prim.pkh.addrs[parseInt(target.dataset.addrIndex!)] = target.value;
      break;
    case "update-hax-hash":
      if (prim.hax)
        prim.hax.hashes[parseInt(target.dataset.hashIndex!)] = target.value;
      break;
  ...
  input.onchange = async (e: Event) => {
    ...
    try {
      ...
      for (const lockData of locksData) {
        ...
        const spendCondition = wasm.SpendCondition.fromProtobuf(
          lockData.spendCondition
        );
        const hash = spendCondition.hash().value;

        // Check for duplicates by hash
        const exists = state.locks.some((l) => {
          const existingSc = wasm.SpendCondition.fromProtobuf(
            l.spendConditionProtobuf
          );
...
downloadTxBtn.onclick = () => {
  console.log("Download TX button clicked");
  console.log("Current state.nockchainTx:", state.nockchainTx);
  ...
  try {
    ...
    const blob = new Blob([new Uint8Array(jamBytes)], {
      type: "application/jam",
    });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
...
document.getElementById("downloadSignedTxBtn")!.onclick = () => {
  ...
  try {
    ...
    const blobRaw = new Blob([new Uint8Array(rawTxBytes)], {
      type: "application/jam",
    });
